...
function adminClearData() {
    ...
    if(d.transactions.length > 100) { // Safety check
        if(confirm('Muitos dados encontrados sem flag de teste. Deseja resetar para o padrão inicial?')) {
...
function runQualityCheck() {
    ...
    d.transactions.forEach(t => {
        if(ids.has(t.id)) log.push(`ERRO: ID Transação duplicado: ${t.id}`);
        ids.add(t.id);
        if(isNaN(t.value)) log.push(`ERRO: Valor inválido na transação ${t.id}`);
        if(!t.date) log.push(`AVISO: Transação sem data ${t.id}`);
    });

    // 2. Verificar Órfãos na Agenda
    d.appointments.forEach(a => {
         if(!a.title) log.push(`AVISO: Agendamento sem título ${a.id}`);
    });

    // 3. Integridade Financeira
    const total = d.transactions.reduce((acc, t) => acc + (t.type==='receita'?t.value:-t.value), 0);
    console.log("Saldo calculado integridade:", total);

    if(log.length === 0) {
        alert("✅ Nenhuma inconsistência encontrada nos dados.");
    } else {
        alert("⚠️ Relatório de Qualidade:\n\n" + log.join("\n"));
    }
}
        ...
        card.innerHTML = `
            ...
            <div class="flex justify-between items-center mt-2 border-t pt-2">
                <div class="text-sm">
                    ...
                    </span> - R$ ${parseFloat(a.value).toFixed(2)}
                </div>
                <div>
...
function deleteAppointment(id) { if(confirm('Excluir este agendamento?')) { const list = getUserData().appointments; list.splice(list.findIndex(a => a.id === id), 1); saveData(); renderAgenda(); } }
...
// --- EXPORTAÇÃO DE ALTA QUALIDADE (PDF e DOCX) ---

function exportRPAPdfHighQuality() {
    // Usa html2pdf para gerar um PDF vetorial de alta qualidade
    const element = document.getElementById('rpa-content');
    const opt = {
        margin: 10,
        filename: 'RPA_Recibo.pdf',
...
function exportRPADocxReal() {
    // Usando docx.js para gerar um arquivo real e editável
    ...
    const doc = new Document({
        sections: [{
            ...
            children: [
                new Paragraph({
                    text: "RECIBO DE PAGAMENTO A AUTÔNOMO (RPA)",
                    heading: HeadingLevel.HEADING_1,
                    alignment: AlignmentType.CENTER
                }),
                new Paragraph({ text: "" }), // Espaço
                new Paragraph({ text: "1. Contratante", heading: HeadingLevel.HEADING_3 }),
                new Paragraph({ text: `Razão: ${document.getElementById('rpa-comp-name').value}` }),
                new Paragraph({ text: `CNPJ: ${document.getElementById('rpa-comp-cnpj').value}` }),
                new Paragraph({ text: "" }),
                new Paragraph({ text: "2. Prestador", heading: HeadingLevel.HEADING_3 }),
                new Paragraph({ text: `Nome: ${document.getElementById('rpa-prov-name').value}` }),
                new Paragraph({ text: `CPF: ${document.getElementById('rpa-prov-cpf').value}` }),
                new Paragraph({ text: "" }),
                new Paragraph({ text: "3. Valores", heading: HeadingLevel.HEADING_3 }),
                new Paragraph({ text: `Serviço: ${document.getElementById('rpa-desc').value}` }),
                new Paragraph({ text: `Valor Bruto: R$ ${document.getElementById('rpa-value').value}` }),
                new Paragraph({ 
                    children: [
                        new TextRun({ text: "Valor Líquido: " + document.getElementById('rpa-net').value, bold: true, size: 28 })
                    ] 
                })
            ]
        }]
    ...
    Packer.toBlob(doc).then(blob => {
        saveAs(blob, "RPA_Recibo.docx");
    });
}

function exportReportDocxReal() {
    // Exportação simplificada para tabela usando blob HTML (Word aceita HTML com extensão .doc/.docx)
    // A qualidade é superior à exportação simples de texto
    const header = `<h2>${appData.currentUser.company.name || "Minha Empresa"}</h2>`; 
    const table = document.getElementById('listing-table').outerHTML; 
    const html = `<html xmlns:o='urn:schemas-microsoft-com:office:office' xmlns:w='urn:schemas-microsoft-com:office:word'><head><meta charset='utf-8'><title>Relatório</title></head><body>${header}<h3>Relatório do Sistema</h3>${table}</body></html>`; 
...
function validateLicense() { 
    ...
    if(k === (c + LIC_PAD_VAL) * LIC_MULT_FACTOR + LIC_YEAR_BASE + d){
        ...
        saveData(); checkLicense(); alert('Ok');
    } else { alert('Erro'); }
}
...
function updateTransactionDependencies(){
    ...
    if (list && list.length > 0) {
        ...
        select.innerHTML = '<option value="">Sem cadastros disponíveis</option>';
    }
}
